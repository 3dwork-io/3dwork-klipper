...
## Description: Macros package, configurations and other utilities for klipper
...
{% hint style="danger" %}  
**Guide in process !!! Although macros are totally functional these are in continuous development.**

**Use them under your own responsibility !!!**  
{% of Endhint%}

Changelog

12/07/2023 - Added support to automate the creation of Bigreteech electronic firmware
...
{% hint style="warning" %}  
In the case that the directory of your Klipper configuration is customized, remember to adjust the first command properly to your installation.  
{% of Endhint%}

{% hint style="info" %}  
In new facilities:

Since Klipper does not allow access to macros until it has a correct printer.cfg and connects with an MCU we can "deceive" Klipper with the following steps that will allow us to use the macros of our Bundle to, for example, launch the Klipper firmware compilation macro if we use a compatible electronics:

-   We assure ourselves that we have our[host as second MCU](raspberry-como-segunda-mcu.md)
-   Next we will add a printer.cfg, remember that these steps is for a clean installation where you do not have any printer.cfg and want to launch the macro to create firmware, such as the one you can see below:
...
With this we can start Klipper to access our macros.  
{% of Endhint%}
...
{% hint style="warning" %}  
**Remember to take the installation step previously if you do not generate an error and you will not be able to start.**
...
-   **Paused instead**, very interesting macros that allow us to make a leisurely scheduled in a layer or launch a command when starting the next layer.   
    ![](../../.gitbook/assets/image%20(143).png)![](../../.gitbook/assets/image%20(1003).png)  
    In addition, another advantage of them is that they are integrated with mainsail with what we will have new functions in our UI as you can see below:  
...
| -preheated of intelligent nozzle in the case of having a probe sensor      |                                                                                                                                                |
| -Possibility of use of Z-Tilt by variable                                  |                                                                                                                                                |
| -Adaptive, forced or from a stored mesh masted                             |                                                                                                                                                |
...
On the other hand we have some**Requirements to implement it (try to add in the logic of the print_Start of the Bundle in the future activating this function by variable and creating a previous and posterior user macro to put user events)**:
...
-   The use of\[save_variables], in our case we will use ~/variables.cfg to store the variables and that is already within the CFG of these macros.   
    This will automatically create a variable file_build_Sheets.cfg where you will keep our disk variables.
...
-   We must include a Apply call_BUILD_SHEET_Adjustment in our print_Start to be able to apply the selected surface zoffset
-   It is important that for the anterior macro, Apply_BUILD_SHEET_Adjustment, work properly we must add a set_Gcode_Offset z = 0.0 just before calling apply_BUILD_SHEET_ADJUSTMENT
...
{% hint style="warning" %}  
It is important that the value in name = "xxxx" coincides with the name we gave when installing our printing surface  
{% of Endhint%}
...
-   **Firmware compilation for supported electronics**, To facilitate the process of creation and maintenance of our Klipper firmware for our MCU we have the macro compile_Firmware that when executing it, we can use our electronics as a parameter to do only this, Klipper will compile for all electronic supported by our Bundle:  
    ![](../../.gitbook/assets/image%20(1540).png)  
    We will find these accessible easily from our UI website in the Firmware Directory_Binaries in our Machine tab (if we use mainsail):  
    ![](../../.gitbook/assets/telegram-cloud-photo-size-4-6019366631093943185-y.jpg)  
    Then you have the list of supported electronic:

**IMPORTANT!!!**

These scripts are prepared to work on a Raspbian system with PI user, if it is not your case you must adapt it.
...
| MKS Gen L            | mks-gen-l                          |
| ZNP Robin Nano DW V2 | ZNP_robin_nano_dw_Category         |
...
{% hint style="info" %}  
It is important that we add these lines at the end of our configuration file ... just above the section so that in the case of macros in our CFG or include these are overwhelmed by ours:  
#\*# \\&lt;---------------------- SAVE_CONFIG ---------------------->  
{% of Endhint%}

{% hint style="warning" %}  
Normal macros have been separated from**macros shell**given that**To enable these it is necessary to make additionally manual steps that are currently testing**y\*\*They may require extra permissions to attribute execution permissions for which the instructions have not been indicated since it is trying to automate.\*\*  
**If you use them it is under your own responsibility.**  
{% of Endhint%}
...
-   **Start Start Gcode_PRINT**, using Placeholders to pass the filament and bed temperature values ​​dynamically:
...
{% hint style="warning" %}  
We must install the plugin[**Post Process Plugin (by frankbags)**](https://gist.github.com/frankbags/c85d37d9faff7bce67b6d18ec4e716ff)From the menu_**Help/Show**_Configuration Folder ... We will copy the previous link script inside the script folder.   
We restart healing and we will go to_**Extensions/Post processing/Modify G-Code**_And we will select_**Mesh Print Size**_.  
{% of Endhint%}  
{ % endtab %}
...
{% hint style="info" %}  
Los**Placeholders are "aka" or variable**of printing.
...
-   **Gcode the Final End_PRINT**, in this case when not using pleaceholders it is common to all laminators
...
To adjust our machine we will use the variables that we will find in Macros/Macros_our_Globals.CFG and that we detail below.
...
Allows you to control what extrusion mode we will use at the end of our start_Print. The value will depend on the configuration of our laminator.

{% hint style="success" %}  
It is advisable to configure your laminator for the use of relative extrusion and adjust this variable to True.  
{% of Endhint%}
...
| Variable                                   | Description                                                                                | Possible values | Default value |
| ------------------------------------------ | ------------------------------------------------------------------------------------------ | --------------- | ------------- |
| variable_preheat_extruder                  | Enable the preheated nozzle at the temperature indicated in variable_preheat_extruder_temp | True / False    | True          |
...
{% hint style="success" %}  
Benefits of using the preheated Nozzle:
...
| Variable                                                                                                           | Description                                                               | Possible values | Default value |
| ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------- | --------------- | ------------- |
| variable_calibrate_bed_mesh                                                                                        | It allows us to select what kind of malle we will use in our start_PRINT: |                 |               |
| -New Mesh, will make us a misery in each impression                                                                |                                                                           |                 |               |
| -StoredMeh, will load a stored mesh and will not perform the bed poll                                              |                                                                           |                 |               |
| -Adaptive, will make us a new misery but adapted to the printing area improving our first layers on many occasions |                                                                           |                 |               |
...
{% hint style="warning" %}  
We advise you to use the adaptive level since it will always adjust the misery to the size of our impression allowing you to have an adjusted malle area.

It is important that we have in our[start -up gcode](../empezamos/configuracion-klipper-en-laminadores.md#configurando-nuestro-laminador-para-usar-nustras-macros-start_print-y-end_print), in the call to our start_Print, print values_MAX y PRINT_MIN.  
{% of Endhint%}
...
| Variable                                                                                                                                              | Description                                     | Possible values | Default value |
| ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | --------------- | ------------- |
| variable_nozzle_priming                                                                                                                               | We can choose between different purity options: |                 |               |
| -Primelline is going to draw the typical purge line                                                                                                   |                                                 |                 |               |
| -Primelineadaptative will generate a purge line that adapts to the area of ​​the printed piece using variable_nozzle_priming_Objectdistance as margin |                                                 |                 |               |
...
-Max will do it at x = max (minus a small safety margin)  
...
-Min will do it at y = 0 (plus a small safety margin)  
-Max will do it at y = max (less a small safety margin)  
-number will be the coordinate and where to locate the purge | min /   
max /   
number | min |  
...
-Auto will go to the center depending on variable_nozzle_prime_start_and | car /   
forwards /   
backwards | auto |
...
| variable_filament_load_length   | Distance in mm to load the new filament ... as in variable_filament_unload_Length We will use the measure from your gear to extruder by adding an extra margin, in this case this extra value will depend on how much you want to be purged ... you can usually give it more margin than the previous value to ensure that it is cleansed the extrusion of the previous filament. | number          | 150           |
| variable_filament_load_speed    | Filament load speed in mm/sec Normally a faster speed is used to discharge.                                                                                                                                                                                                                                                                                                       | number          | 10            |

{% hint style="warning" %}  
Another adjustment necessary for your section\[extruder]the indicated[**max_extrude_only_distance**](https://www.klipper3d.org/Config_Reference.html#extruder)... The advisable value is usually> 101 (if it is not defined uses 50) to for example allow the typical extruder calibration tests.   
...
{% hint style="info" %}  
When using 3DWork settings as a module, they cannot be edited directly from the 3DWork-Klipper directory within your Klipper configuration directory since it will be in Read-only (restricted to only reading) for safety.
...
Simply, what we have to do is paste the macro content\[Gcode_macro GLOBAL_VARS]What can we find in macros/macros_our_Globals.cfg in our printer.cfg.
...
{ % hint style = "warning" %}
The three points (...) of the previous examples are merely to indicate that you can have more configurations between sections ... in no case should they wear.
{ % endhint %}

{% hint style="info" %}

-   We advise you to add comments as you see in the previous case to identify what each section makes
-   Although you do not need to touch all the variables we advise you to copy the entire content of\[Gcode_macro GLOBAL_VARS]{% of Endhint%}
...
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| \_USER_START_PRINT_HEAT_CHAMBER           | It runs just after our enclosure starts heating, if Chamber_Early passes as a parameter to our start_PRINT |
| \_USER_START_PRINT_BEFORE_HOMING          | It is executed before the initial homing for the start of printing                                         |
| \_USER_START_PRINT_AFTER_HEATING_BED      | It runs when our bed arrives at its temperature, before_START_PRINT_AFTER_HEATING_BED                      |
| \_USER_START_PRINT_BED_MESH               | Is launched before_START_PRINT_BED_MESH                                                                    |
| \_USER_START_PRINT_PARK                   | Is launched before_START_PRINT_PARK                                                                        |
| \_USER_START_PRINT_AFTER_HEATING_EXTRUDER | Is launched before_START_PRINT_AFTER_HEATING_EXTRUDER                                                      |

**END_PRINT**

| Macro name                          | Description                                                                         |
| ----------------------------------- | ----------------------------------------------------------------------------------- |
| \_USER_END_PRINT_BEFORE_HEATERS_OFF | It is executed before performing the heater, before_END_PRINT_BEFORE_HEATERS_OFF    |
| \_USER_END_PRINT_AFTER_HEATERS_OFF  | It is executed after the shutdown of the heater, before_END_PRINT_AFTER_HEATERS_OFF |
| \_USER_END_PRINT_PARK               | It is executed before the head of the head, before_END_PRINT_PARK                   |
...
| Macro name                           | Description                                                                                                                                                                          |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| \_START_PRINT_HEAT_CHAMBER           | Heats the enclosure in the event that the Chamber parameter_Early is received by our macro start_Print from the laminator                                                            |
| \_START_PRINT_AFTER_HEATING_BED      | It runs when the bed arrives at temperature, after_USER_START_PRINT_AFTER_HEATING_Bed. Normally, it is used for bed calibration processing (Z_TILT_ADJUST, QUAD_GANTRY_LEVELING,...) |
...
In the following example we will see how in our case we are interested in customizing the parameters of our bed leveling (Bed_mesh) adjusting the survey points (probe_count) regarding the configuration we have by default in the configurations of our Klipper module:
...
In the following example we are going to customize what is the pin of our electronics fan (Controller_fan) to assign it to one different from the default:
