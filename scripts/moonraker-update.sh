...
source /home/pi/printer_data/config/3dwork/scripts/moonraker-ensure-policykit-rules.sh
...
validate_moonraker_config()
{
  ...
  cat /home/pi/printer_data/config/moonraker.conf | grep "\[include 3dwork/moonraker.conf\]" > /dev/null
  has_include=$?
  if [ $has_include -eq 0 ] && [ "$install_version" = "'validate_install': missing" ]; then
    ...
    # cp /home/pi/printer_data/config/3dwork/old-moonraker.conf /home/pi/printer_data/config/3dwork/moonraker.conf
  fi
}
